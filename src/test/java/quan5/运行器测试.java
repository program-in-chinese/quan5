...
public class 运行器测试 {
  ...
  @Test
  public void 混合运算与比较() {
    检验源码求值("1+1==2+1", false);
    检验源码求值("1+1≠2+1", true);
  }

  @Test
  public void 混合运算赋值与比较() {
    // 需要空格分隔'为'与'变量二'
    检验源码求值("变量一=1" + 换行 + "变量二=2" + 换行 + "变量一×2为 变量二", true);
    检验源码求值("变量=1" + 换行 + "变量×2==2", true);
    检验源码求值("先=1" + 换行 + "先 为2", false);

    // 变量名包含关键字, 需要空格分隔'为'与变量名
  ...
  public void 条件() {
    检验源码求值("如果 2>1 3", 3);
    检验源码求值("如果 1>2 3", null);
    检验源码求值("变量=1" + 换行 + "如果 变量<2 变量=变量×2" + 换行 + "变量", 2);
  }
